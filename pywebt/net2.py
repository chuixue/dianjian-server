...
def getNetwork():
    proj = project.getProjectUnit()
    
    fp = open(filePath('network.txt'), 'r')
    lines = fp.readlines()
    fp.close()
...
#        print nodes[sp[0]], nodes[sp[1]]
    ...
    network_filter(G, lsDegree) #为了可视化过滤网络
    
    network = networkx_json(nodes, G)       #网络数据
    
    return json.dumps({'network': network, 'nodes': nodeInfo, 'netInfo': netInfo })
...
def node_format(nodes, lsDegree, lsBetwns, units):
    return map(lambda x : { 'name': x, 'numb': nodes[x], 'degree': lsDegree[nodes[x]] if nodes[x] in lsDegree else 0,
                      'btness': round(lsBetwns[nodes[x]], 3) if nodes[x] in lsBetwns else 0, 'nofit': units[x] if x in units else 0 }, nodes)
#社团信息
